...
NETNAME = "occurencenet-2-f-c-april"
DATANAME = "occurence-2-f-c-very-fine"

LOGFILE = LOGPATH.joinpath(NETNAME, "run1", "cases-log.txt")


def main(dataset_path: Path, net_path: Path):
    elems = dataset_path.stem.split('-')
    mode = RecordMode(elems[0])
    ctx_size = (int(elems[1]) if mode in (RecordMode.OCCURENCES,
                                          RecordMode.CONTEXT)
                else 0)
    pred_path = some(
        net_path.joinpath("run1").glob("test_predictions_epoch*.csv"))
