...
from deeplearn.analysis.util import redirect_stdout, RecordMode
...
def onenet(net_path, datapath):
    ...
    for run_path in filter(Path.is_dir, net_path.glob("*")):
        ...
        for fun in (getfun('summary', summary=True),
            ...
            fun(RecordMode(mode), ctx_size or 0, run_path,
                datapath.joinpath(dataname))


def main(outpath, datapath):
    for net_path in filter(Path.is_dir, outpath.glob("*")):
        onenet(net_path, datapath)


if __name__ == '__main__':
    onenet(OUTPATH.joinpath("occurencenet-2-f-c-april"), DATAPATH)
